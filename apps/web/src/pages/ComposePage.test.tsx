  ...
  RichTextEditor: (props: EditorModule.RichTextEditorProps) => {
    ...
    return (
      ...
      >
        <button
          type="button"
          onClick={() => props.onChange({ type: "doc", content: [{ type: "paragraph" }] })}
        >
          模拟编辑
        </button>
        <button
          ...
          onClick={() =>
            props.onChange({
              ...
              content: [
                ...
                { type: "paragraph", content: [{ type: "text", text: "当前章新编辑" }] },
              ],
            })
          }
        >
        ...
        <button type="button" onClick={() => props.onSubmit?.(props.content)}>
          模拟保存当前章节
        </button>
        <button type="button" onClick={props.onExpand}>
          模拟展开
        </button>
        <button type="button" onClick={() => props.onCommentAnchorOpen?.("thread_1")}>
          模拟间贴锚点
        </button>
      </section>
    );
...
vi.mock("../features/forum/ForumPanels", () => ({
  ChapterRail: (props: {
    ...
    <aside
      ...
      aria-label="章节目录"
      data-chapters={String(props.chapters?.length ?? 0)}
      data-chapter-ids={JSON.stringify(props.chapters?.map((chapter) => chapter.id))}
      data-active-index={String(props.currentIndex ?? 0)}
    >
      <span>模拟章节目录</span>
      <button type="button" onClick={() => props.onSelect(0)}>
        模拟章节 1
      </button>
      {(props.chapters?.length ?? 0) > 1 ? (
        <button type="button" onClick={() => props.onSelect(1)}>
          模拟章节 2
        </button>
      ) : null}
      {props.onToggleHidden ? (
        <button type="button" onClick={() => props.onToggleHidden?.(props.currentIndex ?? 0, true)}>
          模拟隐藏章节
        </button>
      ) : null}
      <button type="button" onClick={() => props.onAddChapter?.()}>
        {props.createArticle ? "创建文章" : "模拟新增章节"}
      </button>
      {props.onDelete ? (
        <>
          <button type="button" onClick={() => void props.onDelete?.(0)}>
            模拟删除章节
          </button>
          <button type="button" onClick={() => void props.onDelete?.(props.currentIndex ?? 0)}>
            模拟删除当前章节
          </button>
        </>
      ) : null}
  ...
  ForumBusinessPanel: (props: {
    onRestore: (revision: number) => void;
    onCompare?: (revision: number) => void;
  }) => (
    <aside>
      <span>模拟创作工具</span>
      <button type="button" onClick={() => props.onCompare?.(17)}>
        模拟比较
      </button>
      <button type="button" onClick={() => props.onRestore(17)}>
        模拟回退
      </button>
    </aside>
  ),
...
vi.mock("../features/comments/CommentThread", () => ({
  CommentThread: (props: { initial: readonly unknown[] }) => (
    <div>模拟回复树 {props.initial.length}</div>
  ),
}));
    ...
    <QueryClientProvider client={client}>
      <AppContext.Provider
        value={{
          identity,
          setIdentity: vi.fn(),
          authMode: authStatus === "authenticated" ? "demo" : "session",
          authStatus,
          login: vi.fn(),
...
const twoChapterDoc: DocumentEnvelope = {
  ...
  content: {
    ...
    content: [
      ...
      {
        type: "heading",
        attrs: { level: 2, chapterStart: false },
        content: [{ type: "text", text: "雾港来信" }],
      },
      {
        type: "heading",
        attrs: { level: 1, chapterStart: true, chapterId: "stable-0" },
        content: [{ type: "text", text: "第一章 潮汐表" }],
      },
      { type: "paragraph", content: [{ type: "text", text: "潮声沿着旧城墙漫上来。" }] },
      {
        type: "heading",
        attrs: { level: 1, chapterStart: true, chapterId: "stable-1" },
...
const chapterRows = (documentId = "demo-post", hasContent = false): ForumChapterItem[] =>
  [0, 1].map((order) => ({
    id: "stable-" + order,
    documentId,
    order,
...
function mockStandaloneChapters() {
  ...
  mocks.getLongTextChapter.mockImplementation(async (documentId: string, id: string) => ({
    ...
    content: {
      type: "doc",
      content: [
        { type: "paragraph", content: [{ type: "text", text: documentId + "/" + id + "正文" }] },
      ],
    },
  }));
}
...
describe("ComposePage", () => {
  beforeEach(() => {
    ...
    mocks.createDocumentChapter
      .mockReset()
      .mockImplementation(
        async (_documentId: string, input: { title: string; chapterId?: string }) => ({
          id: input.chapterId!,
          title: input.title,
          order: 0,
  ...
  it("空库显示创建文章，点击后只创建可编辑的本地空白正文", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "关闭提示" })?.parentElement?.textContent,
    ).toContain("已在本地创建《第一篇文章》，点击保存后上传服务器");
  });
  ...
  it("新建文章先填写名称，并把名称保存在对应本地草稿中", async () => {
    ...
    expect(localStorage.getItem(`ricetext:draft-title:${draftId}`)).toBe("我的第二篇文章");
  });
  ...
  it("目录「删除章节」移除对应章节并提示仅本地生效", async () => {
    ...
    expect(rail).toHaveAttribute("data-chapters", "1");
    expect(screen.getByText(/已删除章节「第一章 潮汐表」/)).toBeInTheDocument();
    // 目录行通过删除章节接口清理（幂等）；身份取自正文标题节点。
    expect(mocks.deleteDocumentChapter).toHaveBeenCalledWith("demo-post", "stable-0");
  });
  ...
  it("记住上次编辑的章节：刷新后仍停留在原章节（移动端）", async () => {
    ...
    expect(screen.getByRole("complementary", { name: "章节目录" })).toHaveAttribute(
      "data-active-index",
      "1",
    );
  });
  ...
  it("切换章节后把当前位置写入本地存储", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "模拟章节 1" }));
    expect(window.localStorage.getItem("ricetext:active-chapter:demo-post")).toBe("0");
  });
  ...
  it("发布前 flush 自动保存，并可关闭成功提示", async () => {
    ...
    // 无内容差异（mock 保存未推进修订）：明确提示未创建章节版本。
    expect(screen.getByText(/内容没有变化，未创建章节版本/)).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "关闭提示" }));
    expect(screen.queryByText(/内容没有变化，未创建章节版本/)).not.toBeInTheDocument();
  });
  ...
  it("在编辑区内比较历史版本并可退出恢复编辑器", async () => {
    ...
    expect(await screen.findByRole("region", { name: "章节版本格式比较视图" })).toBeInTheDocument();
    ...
    fireEvent.click(screen.getByRole("button", { name: "移动" }));
    expect(screen.getByRole("region", { name: "章节版本格式比较视图" })).toBeInTheDocument();
  ...
  it("打开间贴回复树并执行指定版本回退", async () => {
    ...
    await waitFor(() =>
      expect(mocks.getCommentThread).toHaveBeenCalledWith("demo-post", "thread_1"),
    );
    ...
    await waitFor(() =>
      expect(mocks.restoreRevision).toHaveBeenCalledWith("demo-post", "stable-0", 17, 18),
    );
  ...
  it("按正文存在标记加载并保存版本 0 的标准正文", async () => {
    ...
    expect(await screen.findByText("章节已保存为章节版本 1")).toBeInTheDocument();
  });
  ...
  it("从发布章节目录直接删除服务器章节", async () => {
    ...
    await waitFor(() =>
      expect(mocks.deleteDocumentChapter).toHaveBeenCalledWith("demo-post", "uploaded-chapter"),
    );
  ...
  it("发布章节删除失败时保留目录行并显示错误", async () => {
    ...
    await waitFor(() => expect(mocks.getLongTextChapter).toHaveBeenCalled());
    const rail = screen.getByRole("complementary", { name: "章节目录" });
    fireEvent.click(screen.getByRole("button", { name: "模拟删除章节" }));
  ...
  it("没有编辑权限时不提供章节删除入口", async () => {
    ...
    expect(screen.queryByRole("button", { name: "模拟删除章节" })).not.toBeInTheDocument();
  });
  ...
  it("目录数组无序时普通正文编辑保存删除仍绑定服务器实体", async () => {
    ...
    await waitFor(() =>
      expect(mocks.flush).toHaveBeenCalledWith(expect.anything(), expect.any(Number), "stable-1"),
    );
    ...
    await waitFor(() =>
      expect(mocks.deleteDocumentChapter).toHaveBeenCalledWith("demo-post", "stable-1"),
    );
  ...
  it("独立章目录重排后当前实体、正文和保存目标保持一致", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("editor").dataset.content).toContain("demo-post/stable-0正文"),
    );
    fireEvent.click(screen.getByRole("button", { name: "模拟章节 2" }));
    await waitFor(() =>
    ...
    act(() =>
      client.setQueryData(chapterQueryKeys.directory("demo-post"), [
        { ...rows[1]!, order: 0 },
        { ...rows[0]!, order: 1 },
      ]),
    );
  ...
  it("独立正文失败时编辑与快捷保存均被拦截，重试后才允许写入", async () => {
    ...
    mocks.getLongTextChapter.mockResolvedValue({
      ...chapterRows("demo-post", true)[0],
      content: {
        type: "doc",
        content: [{ type: "paragraph", content: [{ type: "text", text: "恢复的正文" }] }],
      },
    });
  ...
  it.each(["保存", "隐藏", "删除"] as const)(
    "%s请求期间切章，迟到成功不会改动新章节界面",
    async (operation) => {
      mockStandaloneChapters();
      const pending = deferred<{ revision: number; deleted: boolean }>();
      const request =
        operation === "保存"
          ? mocks.uploadLongTextChapter
          : operation === "隐藏"
            ? mocks.setDocumentChapterHidden
            : mocks.deleteDocumentChapter;
      ...
      if (operation === "删除")
        mocks.listForumChapters.mockResolvedValue([
          { ...chapterRows("demo-post", true)[1]!, order: 0 },
        ]);
      ...
      expect(invalidate).toHaveBeenCalledWith({
        queryKey: chapterQueryKeys.directory("demo-post"),
      });
      await waitFor(() =>
        expect(
          client.getQueryData<ForumChapterItem[]>(chapterQueryKeys.directory("demo-post")),
        ).toHaveLength(operation === "删除" ? 1 : 2),
      );
    },
  );
  ...
  it("保存期间离开又返回同一章节，旧保存结果仍不能覆盖本次会话", async () => {
    ...
    expect(
      client.getQueryData<{ revision: number }>(chapterQueryKeys.content("demo-post", "stable-0"))
        ?.revision,
    ).toBe(0);
    expect(
      screen.queryByRole("button", { name: "关闭提示" }),
  ...
  it.each(["章节", "文章"] as const)(
    "历史比较期间切换%s，迟到结果不再打开比较界面",
    async (target) => {
      ...
      const articles = ["demo-post", "other"].map((id) => ({
        id,
        title: id,
        revision: 18,
        savedAt: defaultDocument.savedAt,
      ...
      if (target === "章节") fireEvent.click(screen.getByRole("button", { name: "模拟章节 2" }));
        ...
        fireEvent.change(screen.getByRole("combobox", { name: "选择文章" }), {
          target: { value: "other" },
        });
      await waitFor(() =>
        expect(screen.getByTestId("editor")).toHaveAttribute("data-editable", "true"),
      ...
      expect(
        screen.queryByRole("region", { name: "章节版本格式比较视图" }),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: "关闭提示" }),
        screen.queryByRole("button", { name: "关闭提示" })?.parentElement?.textContent ?? "",
    ...
    async (target) => {
      ...
      await act(async () =>
        pending.resolve({
          ...
          content: {
            type: "doc",
            content: [{ type: "paragraph", content: [{ type: "text", text: "迟到的回退正文" }] }],
          },
        }),
      );
