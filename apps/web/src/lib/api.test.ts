...
describe("Web API 客户端", () => {
  ...
  it("读取文章选择列表并按文档查询章节", async () => {
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toBe("/api/forum/chapters?documentId=demo-post");
  });
  ...
  it("网络不可达时保存本地缓存副本并递增较新的修订号", async () => {
    ...
    expect(JSON.parse(localStorage.getItem("ricetext:document:demo-post")!)).toMatchObject({
      revision: 25,
      storage: "local-cache",
    });
  });
  ...
  it("上传最小 steps：PATCH 成功、409 抛出、离线本地应用并缓存", async () => {
    ...
    await expect(
      saveDocumentSteps("demo-post", {
        schemaVersion: 1,
        baseRevision: 18,
        clientMutationId: "steps_2",
        steps,
      }),
    ...
    const cached = JSON.parse(localStorage.getItem("ricetext:document:demo-post")!) as {
  ...
  it("骰子优先使用服务端结果，离线时稳定生成合法结果", async () => {
    ...
    await expect(createDice(" 2d6-1 ", "roll_old")).resolves.toMatchObject({
      rollId: expect.stringMatching(/^tmp_roll_[0-9a-f-]{36}$/u),
      expression: " 2d6-1 ",
      rolls: [3, 3],
      total: 5,
      rerollOf: "roll_old",
  ...
  it("骰子拒绝无效表达式和越界参数，并透传服务端校验错误", async () => {
    ...
    fetchMock.mockRejectedValueOnce(new TypeError("网络离线"));
    await expect(createDice("51d6")).rejects.toMatchObject({
      status: 422,
      message: "骰子数量或面数超出范围",
    });
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse(
        { error: { code: "INVALID_DICE_EXPRESSION", message: "服务端拒绝" } },
        { status: 422 },
      ),
    );
  ...
  it("上传图片成功时不设置 JSON Content-Type，网络失败时生成对象 URL", async () => {
    ...
    await expect(uploadAsset(file)).resolves.toMatchObject({
      assetId: expect.stringMatching(/^tmp_asset_[0-9a-f-]{36}$/u),
      url: "blob:cover",
      name: "cover.png",
    });
  ...
  it("上传 HTTP 失败和超大离线文件均返回可识别错误", async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      jsonResponse(
        { error: { code: "ASSET_TOO_LARGE", message: "图片超过大小限制" } },
        { status: 413 },
      ),
    );
    ...
    fetchMock.mockRejectedValueOnce(new TypeError("网络离线"));
    await expect(uploadAsset(huge)).rejects.toMatchObject({
      status: 422,
      message: "上传限制为 8 MB",
    });
  });
  ...
  it("间贴读取与赞踩覆盖服务器和离线回退", async () => {
    ...
    await expect(voteComment("comment_1", -1)).resolves.toEqual({
      upvotes: 4,
      downvotes: 1,
      myVote: -1,
    });
