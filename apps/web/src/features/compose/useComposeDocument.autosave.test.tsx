...
const longStorage = vi.hoisted(() => ({
  loadLongTextDraft: vi.fn(),
  loadLongTextRaw: vi.fn(),
  saveLongTextDraft: vi.fn(),
  saveLongTextRaw: vi.fn(),
  deleteLongTextValue: vi.fn(),
}));
...
function setup(first = document("a")) {
  ...
  return {
    client,
    ...renderHook(({ id }) => useComposeDocument(id, 0), {
      wrapper,
      initialProps: { id: "a" },
    }),
  };
...
describe("Compose 使用真实 autosave", () => {
  beforeEach(() => {
    ...
    api.createDocumentChapter.mockImplementation(
      async (documentId: string, input: { title: string; order: number }) => ({
        ...input,
        id: `${documentId}-chapter-${input.order}`,
        documentId,
        revision: 0,
  ...
  it("创建 409 接纳已有服务器基线，但本地正文仍须上传 steps", async () => {
    api.saveDocument.mockRejectedValueOnce(
      new ApiError("已存在", 409, undefined, "REVISION_CONFLICT"),
    );
  ...
  it("占位 revision 0 不提前恢复旧草稿，查询完成后按真实基线丢弃", async () => {
    ...
    await act(async () => {
      fetched.resolve(document("uncached", 4));
    });
  ...
  it.each(["server", "offline", "409", "422"])(
    "旧文章迟到 %s 保存不触碰新文章或草稿",
    async (outcome) => {
      ...
      act(() => {
        pending = result.current.autosave.flush();
      });
      ...
      await act(async () => {
        if (outcome === "409" || outcome === "422")
          save.reject(new ApiError("迟到的错误", Number(outcome)));
        else
          save.resolve({
            ...document("a", 2),
            storage: outcome === "offline" ? "local-cache" : "server",
          });
  ...
  it.each(["list", "create"])("切换后迟到目录 %s 不继续注册或触发新会话保存", async (stage) => {
    ...
    await waitFor(() =>
      expect(
        stage === "list" ? api.listForumChapters : api.createDocumentChapter,
  ...
  it("迟到 rollback 不替换新会话正文或 autosave 基线", async () => {
    ...
    act(() => {
      pending = result.current.rollback("chapter-a", 1);
    });
    rerender({ id: "b" });
    await act(async () => {
      restore.resolve(document("a", 3));
      await pending;
    });
  ...
  it("同文章章节操作失效后 rollback 不更新正文、草稿、缓存或普通基线", async () => {
    ...
    let pending!: Promise<DocumentEnvelope>;
    act(() => {
      pending = result.current.rollback("chapter-a", 1, () => capturedChapter === currentChapter);
    });
    currentChapter = 1;
    const restored = { ...document("a", 3), content: content("obsolete rollback") };
    await act(async () => {
      restore.resolve(restored);
      expect(await pending).toEqual(restored);
    });
  ...
  it("有效章节操作的 rollback 仍同时接纳正文和 autosave 基线", async () => {
    ...
    const { result, client } = setup();
    client.setQueryData(chapterQueryKeys.directory("a"), [
      {
        id: "chapter-a",
        documentId: "a",
        title: "章节 A",
    ...
    await act(async () =>
      expect(await result.current.rollback("chapter-a", 1, () => true)).toEqual(restored),
    );
    ...
    expect(
      client.getQueryData<Array<{ id: string; revision: number }>>(chapterQueryKeys.directory("a")),
    ).toEqual(expect.arrayContaining([expect.objectContaining({ id: "chapter-a", revision: 2 })]));
    expect(loadLocalDocumentDraft("a")).toBeNull();
  });
  ...
  it("长文打开、导入、编辑和关闭不暂停普通 autosave 或污染在途保存", async () => {
    ...
    act(() => {
      saving = result.current.compose.autosave.flush();
    });
    await act(async () => {
    ...
    expect(result.current.workspace.captureUploadSnapshot().document.content?.[0]?.type).toBe(
      "longTextBlock",
    );
    expect(result.current.compose.content).toEqual(latest);
    expect(result.current.compose.generation).toBe(2);
    expect(result.current.compose.document).toMatchObject({
      revision: 1,
      content: document("a").content,
    });

    act(() =>
      result.current.workspace.editChapter(result.current.workspace.chapterSummaries[0]!.id, {
        text: "closing long-text buffer",
      }),
    );
    await act(async () => expect(await result.current.workspace.close()).toBe(true));
    expect(result.current.workspace.enabled).toBe(false);
    expect(longStorage.saveLongTextDraft).toHaveBeenLastCalledWith(
      "ricetext:local-long-text:a",
      expect.objectContaining({
        content: expect.arrayContaining([
          expect.objectContaining({
            attrs: expect.objectContaining({ text: "closing long-text buffer" }),
          }),
        ]),
      }),
    );
    ...
    expect(JSON.stringify(api.saveDocumentSteps.mock.calls[1]![1].steps)).not.toContain(
      "longTextBlock",
    );
  ...
  it("长文关闭落盘失败保留长文缓冲，普通正文与服务器基线均不改变", async () => {
    ...
    await act(async () => result.current.workspace.open());
    await act(async () =>
      result.current.workspace.importFile({
    ...
    act(() =>
      result.current.workspace.editChapter(result.current.workspace.chapterSummaries[0]!.id, {
        text: "retained after quota failure",
      }),
    );
    ...
    expect(result.current.workspace.enabled).toBe(true);
    expect(
      result.current.workspace.captureUploadSnapshot().document.content?.[0]?.attrs?.text,
    ).toBe("retained after quota failure");
    ...
    expect(result.current.compose.autosave).toMatchObject({
      revision: 2,
      state: "saved",
      conflictMessage: "",
    });
