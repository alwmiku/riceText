import { expandActiveChapterVolume, groupChaptersByVolume } from "@ricetext/document-core";
...
export function ChapterSidebar({
  ...
  const groups = useMemo(() => groupChaptersByVolume(chapters), [chapters]);

  useEffect(() => {
    setCollapsedVolumes((current) => expandActiveChapterVolume(current, chapters, activeIndex));
  ...
  const canMergeChapter = (index: number) => {
    ...
    return Boolean(
      current && previous && previous.charCount + current.charCount + 2 <= MAX_CHAPTER_LENGTH,
    );
  };
  ...
  return (
    ...
    >
      <div className="mb-[11px] flex items-center justify-between gap-2 text-[13px] font-bold">
        <span>章节列表</span>
        <span className="text-xs font-normal text-muted-foreground">{chapters.length} 章</span>
      </div>
      ...
      {chapters.length === 0 ? (
        ...
        <div className="flex flex-col gap-1">
          {groups.map((group, groupIndex) => {
            const rows = group.items.map(({ chapter, index }) => (
              <div
                ...
                className={cn(
                  ...
                  group.title && "ml-3",
                  index === activeIndex && "bg-[#e2efec] outline outline-1 outline-[#9ccfc6]",
                  dragIndex === index && "opacity-50 outline-dashed outline-1 outline-[#197c73]",
                )}
                role="button"
                tabIndex={0}
                draggable
              ...
              >
                ...
                <div className="flex items-center gap-0.5">
                  ...
                  <button
                    type="button"
                    aria-label="合并到上一章"
                    title={canMergeChapter(index) ? "合并到上一章" : "合并后将超过 50000 字"}
            ...
            const open = !collapsedVolumes.has(group.key);
            return (
              <Collapsible
                key={group.key || `plain-${groupIndex}`}
                open={open}
              ...
              >
                ...
                <CollapsibleContent className="flex flex-col gap-1">{rows}</CollapsibleContent>
              </Collapsible>
            );
          })}
        </div>
