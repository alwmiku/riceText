        ...
        <div className="space-y-3">
          {diff.gaps > 0 ? (
            <div className="rounded-md border border-[#f0b4b0] bg-[#fdf1f0] px-3 py-2 text-xs text-[#8f2b24]">
              本地核对发现仍有 <strong>{diff.gaps}</strong> 段文字未切分进任何章节，建议先核对并处理再上传。
            </div>
          ) : (
          ...
          <p className="text-xs text-muted-foreground">
            将分章上传 <strong>{diff.total}</strong> 个章节（新增 {diff.added}，修改 {diff.modified}）。未变化的章节不会重复上传。
          </p>
          <div className="max-h-64 overflow-auto rounded-md border p-2">
            {diff.rows.map((row, index) => (
              ...
              >
                <span className="truncate">{index + 1}. {row.title}</span>
                <span className={row.status === "未变化" ? "text-muted-foreground" : "text-[#176e66]"}>
                  {row.status}
                </span>
              </div>
            ))}
          ...
          <div className="flex justify-end gap-2">
            <Button size="sm" variant="ghost" disabled={uploading} onClick={() => onOpenChange(false)}>
              取消
            </Button>
