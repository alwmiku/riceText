...
const pendingSuggestions = [
  {
    id: 's1', documentId: 'demo-post', fromText: '渡口的汽笛', toText: '港口的汽笛',
    reason: '与第一章地名保持一致', status: 'pending' as const, authorId: 'reader', reviewerId: null, createdAt: '2026-08-20T08:00:00.000Z',
  },
  {
    id: 's2', documentId: 'demo-post', fromText: '她握紧信封', toText: '她攥紧信封',
    reason: '减少相邻段落用词重复', status: 'pending' as const, authorId: 'wanderer', reviewerId: null, createdAt: '2026-08-20T09:00:00.000Z',
  },
];

const demoPoll = {
  id: 'poll-route', question: '下一章先去哪里？', multiple: false, eligible: true,
  options: [
...
const freeAttachment = {
  id: 'attachment-sample', name: '雾港设定集.txt', mimeType: 'text/plain', price: 10, purchased: false, downloadUrl: null,
};
...
describe('DemoPanels', () => {
  beforeEach(() => {
    ...
    mocks.getPollVotesMock.mockResolvedValue({
      items: [
        { user: { id: 'reader', name: '晚风翻页', role: 'reader' }, optionIds: ['poll-option-tower'], createdAt: '2026-08-20T10:00:00.000Z' },
        { user: { id: 'wanderer', name: '纸页留声', role: 'reader' }, optionIds: ['poll-option-dock'], createdAt: '2026-08-20T11:00:00.000Z' },
      ],
  ...
  it('演示附件购买并展示 70% 作者分成', async () => {
    renderWithQuery(
      <DemoBusinessPanel identity={identities[1]!} documentId="demo-post" baseRevision={18} onRestore={vi.fn()} />,
    );
    ...
    await waitFor(() => expect(mocks.purchaseAttachmentMock).toHaveBeenCalledWith('attachment-sample'));
    expect(await screen.findByRole('button', { name: '已购买，可下载' })).toBeDisabled();
  });

  it('金币不足时禁止购买附件', async () => {
    renderWithQuery(
      <DemoBusinessPanel identity={{ ...identities[1]!, coins: 5 }} documentId="demo-post" baseRevision={18} onRestore={vi.fn()} />,
    );
    fireEvent.click(screen.getByRole('button', { name: '附件' }));
  ...
  it('读者可以选择投票并展开实名明细', async () => {
    ...
    await waitFor(() => expect(mocks.votePollMock).toHaveBeenCalledWith('poll-route', ['poll-option-tower']));
    expect(await screen.findByRole('button', { name: /钟楼.*29 票/ })).toBeInTheDocument();
  ...
  it('加载历史并把目标版本交给回滚回调', async () => {
    ...
    renderWithQuery(<DemoBusinessPanel identity={identities[0]!} documentId="post_7" baseRevision={18} onRestore={onRestore} />);
    fireEvent.click(screen.getByRole('button', { name: '历史' }));

    expect(await screen.findByText('版本 18')).toBeInTheDocument();
    expect(mocks.getRevisionsMock).toHaveBeenCalledWith('post_7', expect.any(AbortSignal));
