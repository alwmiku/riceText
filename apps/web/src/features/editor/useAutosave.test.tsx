...
const changedContent: RichTextNode = { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'changed' }] }] };

function savedDocument(revision: number, storage: DocumentEnvelope['storage'] = 'server'): DocumentEnvelope {
  return { ...defaultDocument, revision, savedAt: `2026-08-20T00:00:0${revision % 10}.000Z`, content: changedContent, storage };
}
...
describe('useAutosave', () => {
  ...
  it('根据文档落点初始化状态，并在文档修订变化时同步', () => {
    const local: DocumentEnvelope = { ...defaultDocument, storage: 'local-demo' };
    const { result, rerender } = renderHook(
      ({ document }) => useAutosave({ document, content: document.content, generation: 0 }),
  ...
  it('停止输入 1.2 秒后保存并通知宿主', async () => {
    ...
    const { result, rerender } = renderHook(
      ({ content, generation }) => useAutosave({ document: defaultDocument, content, generation, onSaved }),
    ...
    expect(saveDocumentMock).toHaveBeenCalledWith('demo-post', expect.objectContaining({
      schemaVersion: 1,
      baseRevision: 18,
      content: changedContent,
      clientMutationId: expect.stringMatching(/^save_/),
    }));
    expect(result.current).toMatchObject({ state: 'saved', revision: 19, savedAt: savedDocument(19).savedAt });
  ...
  it('把同时触发的保存串行化，并让后一请求使用新修订号', async () => {
    ...
    saveDocumentMock.mockReturnValueOnce(first.promise).mockReturnValueOnce(second.promise);
    const { result, rerender } = renderHook(
      ({ content, generation }) => useAutosave({ document: defaultDocument, content, generation }),
      { initialProps: { content: initialContent, generation: 0 } },
    );
    ...
    const newestContent: RichTextNode = { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'newest' }] }] };
    ...
    expect(saveDocumentMock.mock.calls[1]![1]).toMatchObject({ baseRevision: 19, content: newestContent });

    second.resolve({ ...savedDocument(20), content: newestContent });
    await act(async () => { await secondFlush; });
    expect(result.current).toMatchObject({ state: 'saved', revision: 20 });
  });

  it('409 时保留本地内容，接受最新修订后可以继续保存', async () => {
    const { ApiError } = await import('../../lib/api');
    saveDocumentMock.mockRejectedValueOnce(new ApiError('conflict', 409, { latestRevision: 25 }));
    const { result, rerender } = renderHook(
      ({ content, generation }) => useAutosave({ document: defaultDocument, content, generation }),
      { initialProps: { content: initialContent, generation: 0 } },
    );
    ...
    act(() => { result.current.acceptLatest(25); });
    expect(result.current).toMatchObject({ state: 'dirty', revision: 25, conflictMessage: '' });

    saveDocumentMock.mockResolvedValueOnce(savedDocument(26));
    await act(async () => { await result.current.flush(); });
    expect(saveDocumentMock.mock.calls[1]![1]).toMatchObject({ baseRevision: 25 });
    expect(result.current.state).toBe('saved');
  });
  ...
  it('普通异常保持 error 且不循环重试，新内容代次可以再次保存', async () => {
    ...
    expect(result.current).toMatchObject({ state: 'error', conflictMessage: '磁盘暂不可写' });
    await act(async () => { await vi.advanceTimersByTimeAsync(5000); });
    expect(saveDocumentMock).toHaveBeenCalledTimes(1);

    const newestContent: RichTextNode = { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'retry' }] }] };
    saveDocumentMock.mockResolvedValueOnce({ ...savedDocument(19), content: newestContent });
    rerender({ content: newestContent, generation: 2 });
    expect(result.current.state).toBe('dirty');
    await act(async () => { await result.current.flush(); });
