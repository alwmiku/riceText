...
/**
 * 章节身份在**创建时铸造一次**（`chapter_<uuid>`），此后改名、改正文、移动都不变。
 *
...
export async function migrateLongTextChapterIds(document: RichTextNode): Promise<RichTextNode> {
  ...
  for (const node of document.content ?? []) {
    content.push({
      ...
      attrs: {
        ...node.attrs,
        chapterId: isChapterId(node.attrs?.chapterId) ? node.attrs!.chapterId : createChapterId(),
      },
    });
  }
  return {
