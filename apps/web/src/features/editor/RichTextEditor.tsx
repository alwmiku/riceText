...
function cmd(editor: Editor | null, action: (editor: Editor) => boolean): () => void {
  return () => { if (editor) action(editor); };
}
...
function Toolbar({ editor, condensed = false }: { editor: Editor | null; condensed?: boolean }) {
  ...
  if (!editor) return <div className="editor-toolbar h-[46px]" aria-hidden="true" />;
  ...
  const addLink = () => {
    ...
    if (href === null) return;
    if (!/^https?:\/\//i.test(href)) { window.alert('仅允许 HTTP(S) 链接'); return; }
  ...
  return <>
    <div className="editor-toolbar" role="toolbar" aria-label="富文本工具栏">
      <span className="toolbar-group">
        <IconButton label="撤销" onClick={cmd(editor, (value) => value.chain().focus().undo().run())} disabled={!editor.can().undo()}><Undo2 size={16} /></IconButton>
        <IconButton label="重做" onClick={cmd(editor, (value) => value.chain().focus().redo().run())} disabled={!editor.can().redo()}><Redo2 size={16} /></IconButton>
      </span>
      <span className="toolbar-group">
        <IconButton label="加粗" active={editor.isActive('bold')} onClick={cmd(editor, (value) => value.chain().focus().toggleBold().run())}><Bold size={16} /></IconButton>
        <IconButton label="斜体" active={editor.isActive('italic')} onClick={cmd(editor, (value) => value.chain().focus().toggleItalic().run())}><Italic size={16} /></IconButton>
        <IconButton label="下划线" active={editor.isActive('underline')} onClick={cmd(editor, (value) => value.chain().focus().toggleUnderline().run())}><UnderlineIcon size={16} /></IconButton>
        {!condensed && <><IconButton label="一级标题" active={editor.isActive('heading', { level: 1 })} onClick={cmd(editor, (value) => value.chain().focus().toggleHeading({ level: 1 }).run())}><Heading1 size={16} /></IconButton><IconButton label="二级标题" active={editor.isActive('heading', { level: 2 })} onClick={cmd(editor, (value) => value.chain().focus().toggleHeading({ level: 2 }).run())}><Heading2 size={16} /></IconButton></>}
      </span>
      {!condensed && <span className="toolbar-group">
        <select aria-label="字号" className="h-8 w-[72px] rounded border border-input bg-white px-1 text-xs" value={(editor.getAttributes('textStyle').fontSize as string | undefined) ?? '16px'} onChange={(event) => editor.chain().focus().setMark('textStyle', { ...editor.getAttributes('textStyle'), fontSize: event.target.value }).run()}><option>12px</option><option>14px</option><option>16px</option><option>18px</option><option>20px</option><option>24px</option><option>28px</option><option>32px</option></select>
        <select aria-label="字体" className="h-8 w-[86px] rounded border border-input bg-white px-1 text-xs" value={(editor.getAttributes('textStyle').fontFamily as string | undefined) ?? ''} onChange={(event) => event.target.value ? editor.chain().focus().setFontFamily(event.target.value).run() : editor.chain().focus().unsetFontFamily().run()}><option value="">默认字体</option><option value="sans-serif">黑体</option><option value="Noto Serif SC">宋体</option><option value="monospace">等宽</option></select>
        <DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" className="h-8 w-8" aria-label="文字颜色"><span className="h-4 w-4 rounded-sm border border-black/15" style={{ background: (editor.getAttributes('textStyle').color as string | undefined) ?? '#20272c' }} /></Button></DropdownMenuTrigger><DropdownMenuContent className="min-w-0"><div className="flex gap-1 p-1">{colors.map((color) => <button key={color} aria-label={`文字颜色 ${color}`} className="h-7 w-7 rounded border border-black/10" style={{ background: color }} onClick={() => editor.chain().focus().setColor(color).run()} />)}</div></DropdownMenuContent></DropdownMenu>
      </span>}
      {!condensed && <span className="toolbar-group">
        <IconButton label="无序列表" active={editor.isActive('bulletList')} onClick={cmd(editor, (value) => value.chain().focus().toggleBulletList().run())}><List size={16} /></IconButton>
        <IconButton label="有序列表" active={editor.isActive('orderedList')} onClick={cmd(editor, (value) => value.chain().focus().toggleOrderedList().run())}><ListOrdered size={16} /></IconButton>
        <IconButton label="引用" active={editor.isActive('blockquote')} onClick={cmd(editor, (value) => value.chain().focus().toggleBlockquote().run())}><Quote size={16} /></IconButton>
        <IconButton label="左对齐" active={editor.isActive({ textAlign: 'left' })} onClick={cmd(editor, (value) => value.chain().focus().setTextAlign('left').run())}><AlignLeft size={16} /></IconButton>
        <IconButton label="居中" active={editor.isActive({ textAlign: 'center' })} onClick={cmd(editor, (value) => value.chain().focus().setTextAlign('center').run())}><AlignCenter size={16} /></IconButton>
        <IconButton label="右对齐" active={editor.isActive({ textAlign: 'right' })} onClick={cmd(editor, (value) => value.chain().focus().setTextAlign('right').run())}><AlignRight size={16} /></IconButton>
      </span>}
      <span className="toolbar-group">
...
export function RichTextEditor({ content, mode, editable = true, onChange, onSubmit, onExpand, onCommentAnchorOpen, onModeToolsOpen }: RichTextEditorProps) {
  const [mobileToolsOpen, setMobileToolsOpen] = useState(false);
  const extensions = useMemo(() => editorExtensions({
    additionalExtensions: [Placeholder.configure({ placeholder: mode === 'compact' ? '写下回复…' : '开始写作…' })],
  }), [mode]);
  const editor = useEditor({
    ...
    editorProps: {
      attributes: { 'aria-label': mode === 'compact' ? '快速回复编辑区' : '正文编辑区' },
  ...
  if (mode === 'compact') return <div className="compact-shell surface mobile-edge">
    <div className="editor-content-wrap"><EditorContent editor={editor} /></div>
    <div className="compact-actions">
      <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="sm"><MoreHorizontal size={16} />更多<ChevronDown size={13} /></Button></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuItem onSelect={() => onExpand?.()}><TextQuote size={15} />切换完整编辑器</DropdownMenuItem><DropdownMenuItem onSelect={() => onModeToolsOpen?.()}><ImagePlus size={15} />插入图片或骰子</DropdownMenuItem></DropdownMenuContent></DropdownMenu>
      <Button size="sm" onClick={onSubmit}><Send size={15} />发布回复</Button>
    </div>
  </div>;

  if (mode === 'mobile') return <><div className="mobile-editor surface mobile-edge">
    <div className="editor-content-wrap"><EditorContent editor={editor} /></div>
    <div className="mobile-toolbar"><IconButton label="加粗" active={Boolean(editor?.isActive('bold'))} onClick={cmd(editor, (value) => value.chain().focus().toggleBold().run())}><Bold size={18} /></IconButton><IconButton label="插入图片" onClick={() => setMobileToolsOpen(true)}><ImagePlus size={18} /></IconButton><IconButton label="插入骰子" onClick={() => setMobileToolsOpen(true)}><Dice5 size={18} /></IconButton><IconButton label="更多工具" onClick={() => setMobileToolsOpen(true)}><MoreHorizontal size={19} /></IconButton><Button size="icon" aria-label="发布" onClick={onSubmit}><Send size={18} /></Button></div>
