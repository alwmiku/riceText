...
function Toolbar({ editor, condensed = false }: { editor: Editor | null; condensed?: boolean }) {
  ...
  return <>
    <div className="editor-toolbar" role="toolbar" aria-label="富文本工具栏">
      ...
      <span className="toolbar-group">
        <IconButton label="加粗" active={editor.isActive('bold')} disabled={spoilerActive} onClick={cmd(editor, (value) => value.chain().focus().toggleBold().run())}><Bold size={16} /></IconButton>
        <IconButton label="斜体" active={editor.isActive('italic')} disabled={spoilerActive} onClick={cmd(editor, (value) => value.chain().focus().toggleItalic().run())}><Italic size={16} /></IconButton>
