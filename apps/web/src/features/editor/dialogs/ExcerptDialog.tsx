...
import {
  ...
  excerptAttributes,
  excerptParagraphs,
  isExcerptSourceUrlValid,
  isExcerptBatteryValid,
  readerDisplayDefaults,
  type ExcerptValues,
} from "./excerpt-values";
...
function ExcerptDialogForm({
  ...
  const [values, setValues] = useState<ExcerptValues>(() =>
    initial
      ? {
          ...emptyExcerptValues,
          ...initial,
          variant: normalizeNovelExcerptVariant(initial.variant),
        }
      : createExcerptValues(),
  );
  ...
  const platform = READER_PLATFORM_POLICY[normalizeNovelExcerptVariant(values.variant)];
  const validUrl = isExcerptSourceUrlValid(values.sourceUrl);
  const validBattery = isExcerptBatteryValid(values.batteryLevel);
  const canSubmit =
    validUrl && validBattery && (editing || Boolean(values.bookTitle.trim() && values.text.trim()));
    ...
    onChange: (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) =>
      setValues((current) => {
        const next = {
          ...
          [key]:
            key === "variant"
              ? normalizeNovelExcerptVariant(event.target.value)
              : event.target.value,
        };
        if (key === "variant" && !editing) {
          ...
          for (const displayKey of Object.keys(nextDefaults) as Array<keyof typeof nextDefaults>) {
            if (current[displayKey] === previousDefaults[displayKey])
              next[displayKey] = nextDefaults[displayKey];
          }
        }
        return next;
      }),
    ...
    content: [
      {
        type: "novelExcerpt",
        attrs: {
          ...excerptAttributes(values),
          sourceUrl: validUrl ? values.sourceUrl.trim() || null : null,
        },
        ...
        <div className="flex min-w-0 flex-col gap-3">
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="flex min-w-0 flex-col gap-1.5 text-xs font-semibold">
              ...
              <select
                ...
                {...field("variant")}
              >
                {Object.entries(READER_PLATFORM_POLICY).map(([variant, policy]) => (
          ...
          <fieldset className="flex min-w-0 flex-col gap-3">
            ...
            <label
              ...
              data-invalid={!validBattery || undefined}
            >
              电量（%）
              <Input
                type="number"
                ...
                inputMode="numeric"
                {...field("batteryLevel")}
                aria-invalid={!validBattery}
                aria-describedby={!validBattery ? id + "-battery-error" : undefined}
              />
            </label>
            {!validBattery && (
              <p id={id + "-battery-error"} role="alert" className="text-xs text-destructive">
                电量应为 0 至 100 的整数。
              </p>
            )}
            {(platform.headerKind === "reward" || platform.headerKind === "label") && (
              <label className="flex min-w-0 flex-col gap-1.5 text-xs font-semibold">
                顶部信息
                <Input
                  maxLength={80}
                  placeholder={platform.headerLabel}
                  {...field("headerLabel")}
                />
              </label>
            )}
          ...
          <label
            ...
            data-invalid={!validUrl || undefined}
          >
            来源链接（可选）
            <Input
              type="url"
              ...
              aria-describedby={!validUrl ? id + "-url-error" : undefined}
            />
          </label>
          {!validUrl && (
            <p id={id + "-url-error"} role="alert" className="text-xs text-destructive">
          ...
          {!editing && (
            <label className="flex flex-col gap-1.5 text-xs font-semibold">
              ...
              <textarea
                required
                className="max-h-96 min-h-48 w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                {...field("text")}
              />
            </label>
          )}
