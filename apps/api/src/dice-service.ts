...
export class DiceService {
  ...
  /** 解析表达式、投掷一次并保存。 */
  create(expression: string, userId: string): DiceRollResult {
    return this.#roll(expression, userId, null, null);
  }
  ...
  get(rollId: string): DiceRollResult {
    const row = this.#db
      .prepare(
        "SELECT id, root_roll_id, previous_roll_id, expression, details_json, total, created_at FROM dice_rolls WHERE id = ?",
      )
      .get(rollId) as unknown as DiceRow | undefined;
  ...
  #roll(
    ...
    this.#db
      .prepare(
        "INSERT INTO dice_rolls(id, root_roll_id, previous_roll_id, expression, details_json, total, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      )
      .run(
        id,
        root,
  ...
  #map(row: DiceRow): DiceRollResult {
    return {
      ...
      rolls: JSON.parse(row.details_json) as number[],
      total: row.total,
