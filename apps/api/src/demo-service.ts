...
export class DemoService {
  ...
  chapterHashes(documentId: string): Map<string, string | null> {
    const rows = this.#db.prepare("SELECT id, content_hash FROM chapters WHERE document_id = ?").all(documentId) as Array<{ id: string; content_hash: string | null }>;
  ...
  saveChapter(
    ...
    const existing = this.#db.prepare("SELECT revision FROM chapters WHERE id = ? AND document_id = ?").get(chapterId, documentId) as { revision: number } | undefined;
    if (existing && existing.revision !== input.baseRevision)
      throw new HttpError(409, "CHAPTER_REVISION_CONFLICT", "章节已被其他修改更新，请重新对比差异", {
    ...
    this.#db
      ...
      .run(chapterId, input.title, input.order, documentId, revision, JSON.stringify(input.content), input.hash);
