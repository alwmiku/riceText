...
const seedDocument = {
  ...
  content: [
    ...
    { type: "heading", attrs: { level: 1, chapterStart: true }, content: [{ type: "text", text: "楔子 雨季之前" }] },
    ...
    { type: "heading", attrs: { level: 1, chapterStart: true }, content: [{ type: "text", text: "第三章 没有寄件人的信" }] },
    ...
    { type: "heading", attrs: { level: 1, chapterStart: true }, content: [{ type: "text", text: "第四章 待发布" }] },
...
/** 章节身份改为文章 ID + 章节 ID 复合主键，允许跨文章复用章节 ID。 */
const migrationV12 = `
...
SELECT id, title, sort_order, document_id, revision, content_json, content_hash, updated_at, hidden
...
CREATE TABLE suggestions (
  ...
  created_at TEXT NOT NULL,
  reviewed_at TEXT,
  FOREIGN KEY (document_id, chapter_id) REFERENCES chapters(document_id, id)
);
INSERT INTO suggestions(id, document_id, chapter_id, chapter_title, line_no, line_text, from_text, to_text, reason, status, author_id, reviewer_id, created_at, reviewed_at)
SELECT suggestion.id, suggestion.document_id,
  CASE WHEN suggestion.chapter_id IS NOT NULL AND EXISTS (
    SELECT 1 FROM chapters
    WHERE chapters.document_id = suggestion.document_id
      AND chapters.id = suggestion.chapter_id
  ) THEN suggestion.chapter_id ELSE NULL END,
  suggestion.chapter_title, suggestion.line_no, suggestion.line_text,
  suggestion.from_text, suggestion.to_text, suggestion.reason, suggestion.status,
  suggestion.author_id, suggestion.reviewer_id, suggestion.created_at, suggestion.reviewed_at
...
CREATE INDEX suggestions_chapter_idx ON suggestions(document_id, chapter_id, status, created_at DESC);
`;

/** 长文本整本上传会话：暂存完整章节集，验收后原子替换线上目录。 */
const migrationV13 = `
...
CREATE INDEX chapter_uploads_manifest_idx ON chapter_uploads(document_id, manifest_hash, status, created_at DESC);
CREATE TABLE chapter_upload_items (
  ...
  FOREIGN KEY (document_id, upload_id) REFERENCES chapter_uploads(document_id, id) ON DELETE CASCADE
);
...
export function createDatabase(options: DatabaseOptions): DatabaseSync {
  ...
  runMigration(db, 15, migrationV15);
