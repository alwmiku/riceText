...
**拉到带新 migration 的提交后必须重新执行上面这条命令**：`pnpm.cmd cf:seed:local` 会先
`d1 migrations apply --local` 再播种。只重启 `pnpm.cmd dev` 不会补表，新接口会因为
「no such table」在浏览器里显示「服务器处理请求时发生错误」。
...
文章标签属于**整篇文章**而不是章节，写在 `document_tags`：`tags` 是版主维护的站点字典（`slug` 稳定不可改，隐藏只做软隐藏），作者手动输入 `#标签` 时命中未隐藏的字典条目就归为服务器标签，否则建成只属于这篇文章的作者标签，不会自动写进字典；一篇文章最多 5 个标签。标签是元数据，写入不产生新修订、不改变 `current_revision`，与正文的乐观并发互不影响。
