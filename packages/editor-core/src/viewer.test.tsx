...
import { isRevisionSurfaceProse } from "./revision-region.js";
    ...
    <RichTextViewer
      content={{
        ...
        content: [
          {
            type: "paragraph",
            attrs: { firstLineIndent: 2, leftIndent: 4 },
  ...
  content: [
    {
      ...
      content: [
        ...
        { type: "hardBreak" },
        {
          ...
          attrs: { threadId: "thread-1", count: 3, placement: "end" },
        },
        {
          type: "mention",
          attrs: { userId: "u1", name: "Lin", resolved: true, avatarUrl: "/uploads/avatar.png" },
        },
        {
          type: "diceRoll",
          attrs: {
    ...
    {
      type: "replyGate",
      attrs: { gateId: "gate-1", prompt: "Reply to unlock" },
...
describe("RichTextViewer", () => {
      ...
      <RichTextViewer
        content={{
          ...
          content: [
            {
              type: "paragraph",
              content: [{ type: "text", text: "Read only", marks: [{ type: "bold" }] }],
            },
          ],
        }}
      />,
  ...
  it("renders empty comment anchors as small gray bubbles without a number", async () => {
          ...
          content: [
            {
              ...
              content: [
                {
                  type: "inlineCommentAnchor",
                  attrs: { threadId: "thread-0", count: 0, placement: "end" },
                },
              ],
            },
          ],
    ...
    await waitFor(() =>
      expect(container.querySelector(".rt-inline-comment-anchor--empty")).not.toBeNull(),
    );
          ...
          content: [
            {
              ...
              content: [
                {
                  type: "mention",
                  attrs: { userId: "u1", name: "Lin", resolved: true, avatarUrl: null },
                },
      ...
      <RichTextViewer
        ...
        content={{
          ...
          content: [
            {
              type: "replyGate",
              attrs: { gateId: "g1", prompt: "Reply first" },
              ...
              content: [
                {
                  ...
                  marks: [
                    ...
                    {
                      type: "textStyle",
                      attrs: { color: "#0f766e", fontFamily: "serif", fontSize: "18px" },
                    },
                  ],
                },
              ],
  ...
  it("dispatches inline comments, dice, mentions, reply gates, attachments, polls, links, and spoilers", async () => {
    ...
    render(
      <RichTextViewer
        ...
        interactions={{
          ...
          onReplyGateRequest,
          getAttachmentState: (attrs) => ({
            available: attrs.attachmentId === "file-2",
            pending: false,
          }),
          onAttachmentActivate,
          getPollState: () => ({
            selectedOptionIds: [],
            votesByOption: { one: 4, two: 7 },
            canVote: true,
            pending: false,
          }),
          onPollVote,
    ...
    expect(onInlineCommentActivate).toHaveBeenCalledWith(
      expect.objectContaining({ threadId: "thread-1" }),
    );
    expect(onDiceReroll).toHaveBeenCalledWith(
      expect.objectContaining({ rollId: "roll-1", total: 12 }),
    );
    ...
    expect(onAttachmentActivate).toHaveBeenCalledWith(
      expect.objectContaining({ attachmentId: "file-1" }),
    );
    ...
    const chartBars = document.querySelectorAll(".rt-poll__chart-bar-stack");
    ...
    expect(screen.getByRole("button", { name: /One\s*4 ballots/u })).toHaveAttribute(
      "aria-pressed",
      "true",
    );
  ...
  it("opens the gallery and supports toolbar, keyboard, wheel, and drag controls", async () => {
    ...
    render(
      <RichTextViewer
        content={interactiveDocument}
        interactions={{ onImageOpen }}
        labels={{ closeImage: "Dismiss", nextImage: "Forward", previousImage: "Back" }}
      />,
    );
    ...
    await waitFor(() =>
      expect(screen.getByRole("dialog").querySelector("img")).toHaveStyle({
        transform: "translate(20px, 30px) scale(1.25)",
      }),
    );
  ...
  it("按扩展声明标记修订面：装饰区域不参与读者修订", async () => {
    const { container } = render(
      <RichTextViewer enableLightbox={false} content={interactiveDocument} />,
    );
    await screen.findByText("Choose one");
    const viewer = container.querySelector(".rt-viewer .tiptap.ProseMirror")!;
    const markerOf = (element: Element | null | undefined) =>
      element?.closest("[data-rt-revise]")?.getAttribute("data-rt-revise");
    const prose = (target: Node) => isRevisionSurfaceProse(viewer, target);

    // 投影契约：装饰节点带 chrome，声明为正文的容器带 prose，条件区域带 when。
    expect(markerOf(screen.getAllByText("Choose one")[0])).toBe("chrome");
    expect(markerOf(screen.getAllByText("Excerpt body")[0])).toBe("prose");
    expect(markerOf(container.querySelector('[data-spoiler="true"]'))).toBe("when");

    // 判定语义：只有持久化为 text 节点的正文可修订。
    expect(prose(screen.getByText("source"))).toBe(true);
    for (const text of ["Choose one", "One", "archive.zip", "Alpha caption", "@Lin", "3d5"]) {
      expect(prose(screen.getAllByText(text)[0]!), text).toBe(false);
    }
    expect(prose(screen.getByText("Excerpt body"))).toBe(true);
    expect(prose(screen.getAllByText("Chapter 1")[0]!)).toBe(false);
    expect(prose(container.querySelector(".rt-reader-progress")!)).toBe(false);
    expect(prose(container.querySelector('[data-spoiler="true"]')!)).toBe(false);
    // 锁定态回复门控渲染成按钮，即使容器声明为正文也由平台控件规则排除。
    expect(prose(screen.getByText("Reply to unlock"))).toBe(false);
  });

  it("clamps and resets the reusable viewer controller", async () => {
    const { result, rerender } = renderHook(({ count }) => useRichTextViewerController(count), {
