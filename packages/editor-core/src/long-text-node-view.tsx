 ...
 */
export function LongTextView({ node, updateAttributes, editor, getPos }: NodeViewProps) {
  ...
  if (!editor.isEditable) {
    return (
      <NodeViewWrapper as="section" className="rt-long-text" data-node-type="long-text-block" contentEditable={false}>
        <div className="rt-long-text__header">
          <strong className="rt-long-text__title">{attrs.title || '未命名章节'}</strong>
          <span className="rt-long-text__meta">{value.length.toLocaleString()} 字</span>
        </div>
  ...
  return (
    <NodeViewWrapper as="section" className="rt-long-text" data-node-type="long-text-block" contentEditable={false}>
      <div className="rt-long-text__header">
        ...
        />
        <span className="rt-long-text__meta">{value.length.toLocaleString()} 字</span>
        <button type="button" className="rt-long-text__split" onClick={splitHere}>
          从这里切分章节
        </button>
      </div>
      <textarea
