...
describe('document sanitization', () => {
  ...
  it('normalizes safe colors, fonts, and pixel sizes', () => {
    ...
    expect(sanitizeFontFamily('Noto Serif SC Variable')).toBe('Noto Serif SC Variable')
    ...
    expect(sanitizeFontSize('48px')).toBe('48px')
  ...
  it('normalizes attributes for every persisted business node', () => {
    ...
    // heading 属性键序与 schema 注册顺序（对齐、章节标记、缩进、标题等级、章节身份）一致，
    // 保证重建文档与编辑器 PM 序列化结果 JSON 完全相等。
    expect(result.document.content?.[0]?.attrs).toEqual({ textAlign: 'justify', chapterStart: false, firstLineIndent: 0, leftIndent: 0, level: 6, chapterId: null })
