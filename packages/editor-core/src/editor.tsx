...
import { ALLOWED_FONT_FAMILIES, ALLOWED_FONT_SIZES, sanitizeDocument, sanitizeUrl } from './sanitize.js'
...
function ToolbarButton({ label, active = false, disabled = false, children, onPress }: ToolbarButtonProps) {
  return (
    <button
      type="button"
 ...
 */
export function EditorToolbar({ editor, mode = 'full', disabled = false, onSubmit, ...requests }: EditorToolbarProps) {
  ...
  return (
    <div className={`rt-toolbar rt-toolbar--${mode}`} role="toolbar" aria-label="Rich text formatting">
      <div className="rt-toolbar__group">
        <ToolbarButton label="Undo" disabled={preventDisabled || !editor.can().undo()} onPress={() => editor.chain().focus().undo().run()}>↶</ToolbarButton>
      ...
      <div className="rt-toolbar__group">
        ...
        <ToolbarButton label="Link" active={editor.isActive('link')} disabled={preventDisabled} onPress={requests.onRequestLink ? request(requests.onRequestLink) : () => setDefaultLink(editor)}>↗</ToolbarButton>
      </div>
      {mode !== 'compact' ? (
        <>
          <div className="rt-toolbar__group">
            <label className="rt-toolbar__field">
              ...
              >
                <option value="p">Text</option><option value="h1">H1</option><option value="h2">H2</option><option value="h3">H3</option>
              </select>
            </label>
            <label className="rt-toolbar__field">
          ...
          <div className="rt-toolbar__group">
            <ToolbarButton label="Bullet list" active={editor.isActive('bulletList')} disabled={preventDisabled} onPress={() => editor.chain().focus().toggleBulletList().run()}>•≡</ToolbarButton>
            <ToolbarButton label="Numbered list" active={editor.isActive('orderedList')} disabled={preventDisabled} onPress={() => editor.chain().focus().toggleOrderedList().run()}>1≡</ToolbarButton>
            <ToolbarButton label="Block quote" active={editor.isActive('blockquote')} disabled={preventDisabled} onPress={() => editor.chain().focus().toggleBlockquote().run()}>❝</ToolbarButton>
            <ToolbarButton label="Align left" active={editor.isActive({ textAlign: 'left' })} disabled={preventDisabled} onPress={() => editor.chain().focus().setTextAlign('left').run()}>≡</ToolbarButton>
            <ToolbarButton label="Align center" active={editor.isActive({ textAlign: 'center' })} disabled={preventDisabled} onPress={() => editor.chain().focus().setTextAlign('center').run()}>≣</ToolbarButton>
            <ToolbarButton label="Align right" active={editor.isActive({ textAlign: 'right' })} disabled={preventDisabled} onPress={() => editor.chain().focus().setTextAlign('right').run()}>≡</ToolbarButton>
          </div>
          <div className="rt-toolbar__group">
            {requests.onRequestComment ? <ToolbarButton label="Inline comment" disabled={preventDisabled} onPress={request(requests.onRequestComment)}>☵</ToolbarButton> : null}
            {requests.onRequestImage ? <ToolbarButton label="Insert image" disabled={preventDisabled} onPress={request(requests.onRequestImage)}>▧</ToolbarButton> : null}
            {requests.onRequestDice ? <ToolbarButton label="Roll dice" disabled={preventDisabled} onPress={request(requests.onRequestDice)}>⚄</ToolbarButton> : null}
            {requests.onRequestNovelExcerpt ? <ToolbarButton label="Novel excerpt" disabled={preventDisabled} onPress={request(requests.onRequestNovelExcerpt)}>▤</ToolbarButton> : null}
            {requests.onRequestMention ? <ToolbarButton label="Mention user" disabled={preventDisabled} onPress={request(requests.onRequestMention)}>@</ToolbarButton> : null}
            {requests.onRequestReplyGate ? <ToolbarButton label="Reply-gated content" disabled={preventDisabled} onPress={request(requests.onRequestReplyGate)}>◉</ToolbarButton> : null}
            {requests.onRequestAttachment ? <ToolbarButton label="Attachment" disabled={preventDisabled} onPress={request(requests.onRequestAttachment)}>⌕</ToolbarButton> : null}
            {requests.onRequestPoll ? <ToolbarButton label="Poll" disabled={preventDisabled} onPress={request(requests.onRequestPoll)}>▥</ToolbarButton> : null}
          </div>
        </>
      ) : null}
      {onSubmit ? <button type="button" className="rt-toolbar__submit" disabled={preventDisabled} onClick={() => void onSubmit(sanitizeDocument(editor.getJSON()), editor)}>Submit</button> : null}
    </div>
  )
}
...
export function RichTextEditor({
  ...
  const configuredExtensions = useMemo(() => editorExtensions(extensions ? { additionalExtensions: extensions } : {}), [extensions])
  ...
  useEffect(() => {
    ...
    if (JSON.stringify(editor.getJSON()) !== JSON.stringify(safe)) editor.commands.setContent(safe, { emitUpdate: false })
  }, [content, editor])
  ...
  if (!editor) return <div className={`rt-editor rt-editor--${mode} ${className}`} aria-busy="true" />

  const toolbarMode = mode === 'compact' && compactExpanded ? 'full' : mode
  const toolbar = renderToolbar?.(editor) ?? (
    <EditorToolbar editor={editor} mode={toolbarMode} disabled={!editable} {...requests} {...(onSubmit && mode !== 'compact' ? { onSubmit } : {})} />
  )
  return (
    <section className={`rt-editor rt-editor--${mode} ${className}`} data-editor-mode={mode}>
      {mode === 'compact' ? (
        <div className="rt-editor__compact-actions">
          {status ? <div className="rt-editor__status">{status}</div> : null}
          {onSubmit ? <button type="button" className="rt-toolbar__submit" disabled={!editable} onClick={() => void onSubmit(sanitizeDocument(editor.getJSON()), editor)}>Submit</button> : null}
          <button type="button" className="rt-editor__expand" aria-expanded={compactExpanded} onClick={() => setCompactExpanded((value) => !value)}>
      ...
      {mode !== 'compact' && status ? <div className="rt-editor__status">{status}</div> : null}
    </section>
  )
}
