...
function chrome(spec: DOMOutputSpec, key: number): ReactNode {
  ...
  return createElement(
    (tag as string).split(" ").at(-1)!,
    { ...props, key },
    ...children.map((child, index) => chrome(child as DOMOutputSpec, index)),
  );
}
/** 选区是否落在该摘录内：整节点被选中，或光标在摘录正文里。 */
function selectionInsideExcerpt(
  editor: Editor | null,
...
function useExcerptActive(
  ...
  const getSnapshot = useCallback(
    () => selectionInsideExcerpt(editor, getPosRef.current),
    [editor],
  );
  return useSyncExternalStore(subscribe, getSnapshot, () => false);
}

export function NovelExcerptNodeView({
  attrs,
  editable = false,
  editor = null,
  getPos,
  ...
  return (
    <NodeViewWrapper
      as="aside"
      className={
        "rt-novel-excerpt rt-novel-excerpt--" +
        variant +
        (active ? " rt-novel-excerpt--active" : "")
      }
      data-node-type="novel-excerpt"
      data-variant={variant}
      data-page-index={pagination.pages.index}
      data-page-count={pagination.pages.count}
      data-book-title={attrs.bookTitle}
      data-chapter-title={attrs.chapterTitle}
      data-author={attrs.author}
      data-source-url={sourceUrl ?? ""}
      data-reader-time={attrs.readerTime}
      data-battery-level={attrs.batteryLevel}
      data-page-label={attrs.pageLabel}
      data-progress-label={attrs.progressLabel}
      data-header-label={attrs.headerLabel}
      data-empty-bubble={String(READER_PLATFORM_POLICY[variant].emptyBubble)}
    >
      <div
        className="rt-reader-page"
        ref={pagination.pageRef}
        data-paginated={pagination.paginated}
      >
        {readerTop(raw).map(chrome)}
        <div
          className="rt-reader-viewport"
          ref={pagination.viewportRef}
          role={pagination.paginated ? "group" : undefined}
          aria-label={pagination.paginated ? "摘录分页正文" : undefined}
          tabIndex={pagination.paginated ? 0 : undefined}
          onPointerDown={pagination.onPointerDown}
          onPointerUp={pagination.onPointerUp}
          onPointerCancel={pagination.onPointerCancel}
          onKeyDown={pagination.onKeyDown}
        >
          <NodeViewContent
            className="rt-novel-excerpt__content"
            style={{
              transform: pagination.paginated
                ? `translateX(calc(-${pagination.pages.index} * (var(--reader-column-width, 100%) + var(--reader-column-gap, 24px))))`
                : undefined,
            }}
          />
        </div>
