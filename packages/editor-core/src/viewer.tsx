...
export function RichTextViewer({
  ...
  useEffect(() => {
    ...
    const decorate = () => {
      ...
      // 按扩展声明标记修订面：装饰区域不参与读者修订。
      applyRevisionSurfaces(editor);
    };
    decorate();
    editor.on("create", decorate);
    return () => {
      editor.off("create", decorate);
    };
  }, [editor, document]);
  ...
  useEffect(() => {
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      if (link?.getAttribute("href")) {
        ...
        current.interactions.onLinkActivate?.(
          href,
  ...
  return (
    <>
      ...
      {enableLightbox ? (
        <ImageLightbox controller={controller} images={gallery.images} labels={labels} />
      ) : null}
    </>
  );
}
