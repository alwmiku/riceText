...
export interface RiceTextApiClient {
  ...
  updateDocument(
    ...
    body: {
      ...
      title?: string;
      content: DocumentEnvelope["content"];
    },
    signal?: AbortSignal,
  ): Promise<DocumentEnvelope>;
  ...
  deleteDocumentChapter(
    ...
    chapterId: string,
    signal?: AbortSignal,
  ): Promise<z.infer<typeof DeleteDocumentChapterResponseSchema>>;
  ...
  createChapterUpload(
    novelId: string,
    body: { manifestHash: string; totalChapters: number },
    signal?: AbortSignal,
  ): Promise<{
    uploadId: string;
    manifestHash: string;
  ...
  listChapters(
    documentId: string,
    signal?: AbortSignal,
  ): Promise<{ items: Array<z.infer<typeof ChapterSchema>> }>;
  /** 按名称或 ID 搜索好友/用户。 */
  searchUsers(
    query: string,
    friendsOnly?: boolean,
    signal?: AbortSignal,
  ): Promise<{ items: ForumUser[] }>;
  ...
  resolveReplyGate(
    ...
    documentId: string,
    signal?: AbortSignal,
  ): Promise<z.infer<typeof ResolveReplyGateResponseSchema>>;
  /** 读取当前身份可见的章节纠错建议。 */
  listSuggestions(
    documentId: string,
    signal?: AbortSignal,
  ): Promise<{ items: Array<z.infer<typeof SuggestionSchema>> }>;
  /** 提交一条带章节和行定位的待审核纠错建议。 */
  createSuggestion(
    documentId: string,
    body: {
  ...
  /** 审核纠错建议；approve 在服务端章节范围核对行上下文后唯一替换，旧建议须全文唯一；定位失效或歧义返回 409。 */
  reviewSuggestion(
    suggestionId: string,
    body: { decision: "approve" | "reject"; baseRevision: number },
    signal?: AbortSignal,
  ): Promise<{
  ...
  listSuggestionBatches(
    documentId: string,
    signal?: AbortSignal,
  ): Promise<{ items: Array<z.infer<typeof SuggestionBatchSchema>> }>;
  /** 提交一个整章校订批次。 */
  createSuggestionBatch(
    ...
    body: {
      ...
      chapterId: string;
      chapterTitle: string;
      beforeContent: DocumentEnvelope["content"];
      afterContent: DocumentEnvelope["content"];
      steps: Array<Record<string, unknown>>;
      reason: string;
    },
    signal?: AbortSignal,
  ): Promise<z.infer<typeof SuggestionBatchSchema>>;
  /** 原子审核整章校订批次。 */
  reviewSuggestionBatch(
    ...
    batch: z.infer<typeof SuggestionBatchSchema>;
    document: DocumentEnvelope | null;
  }>;
  /** 读取附件价格和当前购买权益。 */
  getAttachment(
...
export function createApiClient(
  ...
  return {
    ...
    updateDocument: (id, body, signal) =>
      request(`/api/documents/${id}`, {
        method: "PUT",
        body: json(body),
        signal,
      }),
    updateDocumentSteps: (id, body, signal) =>
      request(`/api/documents/${id}/steps`, {
        method: "PATCH",
        body: json(body),
        signal,
      }),
    createDocumentChapter: (id, body, signal) =>
      request(`/api/documents/${id}/chapters`, {
        method: "POST",
        body: json(body),
        signal,
      }),
    deleteDocumentChapter: (id, chapterId, signal) =>
      request(`/api/documents/${id}/chapters/${chapterId}`, {
        method: "DELETE",
        signal,
      }),
    updateDocumentChapter: (id, chapterId, body, signal) =>
      request(`/api/documents/${id}/chapters/${chapterId}`, {
        method: "PATCH",
        body: json(body),
        signal,
      }),
    syncNovelChapters: (novelId, chapters, signal) =>
      request(`/api/forum/novels/${novelId}/chapters/sync`, {
        method: "POST",
        body: json({ chapters }),
        signal,
      }),
    getNovelChapter: (novelId, chapterId, signal) =>
      request(`/api/forum/novels/${novelId}/chapters/${chapterId}`, { signal }),
    saveNovelChapter: (novelId, chapterId, input, signal) =>
      request(`/api/forum/novels/${novelId}/chapters/${chapterId}`, {
        method: "PUT",
        body: json(input),
        signal,
      }),
    saveNovelChaptersBatch: (novelId, body, signal) =>
      request(`/api/forum/novels/${novelId}/chapters/batch`, {
        method: "POST",
        body: json(body),
        signal,
      }),
    createChapterUpload: (novelId, body, signal) =>
      request(`/api/forum/novels/${novelId}/chapter-uploads`, {
        method: "POST",
        body: json(body),
        signal,
      }),
    ...
    createDice: (expression, rerollOf, signal) =>
      request("/api/dice", {
        method: "POST",
        body: json({ expression, ...(rerollOf ? { rerollOf } : {}) }),
        signal,
      }),
    ...
    createCommentReply: (documentId, anchorId, body, parentId, signal) =>
      request(`/api/documents/${documentId}/comments/${anchorId}/replies`, {
        method: "POST",
        body: json({ body, parentId: parentId ?? null }),
        signal,
      }),
    voteComment: (replyId, value, signal) =>
      request(`/api/comments/replies/${replyId}/vote`, {
        method: "PUT",
        body: json({ value }),
        signal,
      }),
    ...
    searchUsers: (q, friendsOnly = false, signal) =>
      request(`/api/forum/users/search${query({ q, friendsOnly })}`, {
        signal,
      }),
    resolveMention: (name, userId, signal) =>
      request("/api/forum/mentions/resolve", {
        method: "POST",
        body: json({ name, ...(userId ? { userId } : {}) }),
        signal,
      }),
    resolveReplyGate: (gateId, documentId, signal) =>
      request("/api/forum/reply-gates/resolve", {
        method: "POST",
    ...
    reviewSuggestion: (id, body, signal) =>
      request(`/api/forum/suggestions/${id}`, {
        method: "PATCH",
        body: json(body),
        signal,
      }),
    listSuggestionBatches: (id, signal) =>
      request(`/api/forum/documents/${id}/suggestion-batches`, { signal }),
    createSuggestionBatch: (id, body, signal) =>
      request(`/api/forum/documents/${id}/suggestion-batches`, {
        method: "POST",
        body: json(body),
        signal,
      }),
    reviewSuggestionBatch: (id, body, signal) =>
      request(`/api/forum/suggestion-batches/${id}`, {
        method: "PATCH",
        body: json(body),
        signal,
      }),
    ...
    submitPollVote: (id, optionIds, signal) =>
      request(`/api/forum/polls/${id}/votes`, {
        method: "POST",
        body: json({ optionIds }),
        signal,
      }),
